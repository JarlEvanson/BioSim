...
impl Shader {
    pub fn new(vertexPath: &str, fragmentPath: &str) -> Shader {
        let mut vertexFile =
            std::fs::File::open(vertexPath).expect("Failed to open Vertex Shader File");
        let mut fragmentFile =
            std::fs::File::open(fragmentPath).expect("Failed to open Fragment Shader File");

        let mut vertexString = Vec::new();
        let mut fragmentString = Vec::new();

        vertexFile
            .read_to_end(&mut vertexString)
            .expect("Failed to read vertex file");
        fragmentFile
            .read_to_end(&mut fragmentString)
            .expect("Failed to read fragment file");

        let vertexString =
            std::ffi::CString::new(vertexString).expect("Failed to convert vertex file data");
        let fragmentString =
        ...
        unsafe {
            let location = gl::GetUniformLocation(
                ...
                std::ffi::CString::new(name)
                    .expect("Failed to convert uniform location")
                    .as_ptr(),
            );
